...
const Sidebar = () => {
  ...
  const [showMenu, setShowMenu] = useState(false);     //added line
  ...
  return (
    <div  className={`bg-[#8185B2]/10 px-3 py-5 h-full rounded-r-xl  text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
        ...
        {filteredUsers.map((user ,index )=>(
          <div onClick={()=>{
           ...
           key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}> 
            ...
            {unseenMessages[user._id] > 0 && (<p className="absolute top-4 right-2 text-xs h-5 w-5 flex justify-center items-center rounded-full bg-violet-500/50 text-white"> {unseenMessages[user._id]} </p>
            )}
