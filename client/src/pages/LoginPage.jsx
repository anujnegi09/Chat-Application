...
const LoginPage = () => {
  ...
  return (
    <div style={{ backgroundImage: `url(${assets.bgImage})` }} className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
     {/* ------------left------------- */}
...
/>
         ...
         )}
         {
          currState === "sign up" && isDataSubmitted && ( 
            ...
            rows={4} required className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500' placeholder='provide a short bio'></textarea>
          )
         }
         ...
         <div className='flex flex-col gap-2'>
          {currState === "sign up" ? (
            <p className='text-sm text-gray-600'>Already have an account ? <span onClick={()=>{
              setCurrState("Login")
              ; setIsDataSubmitted(false)}}
              className='font-medium active:scale-95 text-violet-500 cursor-pointer'>Login here</span></p>
          ) :(
            <p className='text-sm active:scale-95 text-gray-600'>Create an account <span onClick={()=>{
